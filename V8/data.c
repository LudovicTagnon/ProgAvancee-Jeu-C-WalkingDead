...
void init_data(world_t *world) {
    ...
    printf("ERROR2\n");
    //initialise vaisseau
    init_sprite_ptr(&world->Rick, SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, RICK_WIDTH, RICK_HEIGHT, 50);
    //=======================================================================
    ...
    int x = world->Rick.position.x;
    int y = world->Rick.position.y;

    sprite_t tmp_sprite;
    init_sprite(tmp_sprite, x, y, 16, 16, 0);
    world->nb_bullets++;
...
int sprites_collide(sprite_t *sp1, sprite_t *sp2) {
    ...
    if ((((s1.position.x - s1.w / 2 <= s2.position.x + s2.w / 2) &&
          (s2.position.x + s2.w / 2 <= s1.position.x + s1.w / 2)) ||
         ((s1.position.x - s1.w / 2 <= s2.position.x - s2.w / 2) &&
          (s2.position.x - s2.w / 2 <= s1.position.x + s1.w / 2)))
        && ((((s1.position.y - s1.h / 2 <= s2.position.y + s2.h / 2) &&
              (s2.position.y + s2.h / 2 <= s1.position.y + s1.h / 2)) ||
             ((s1.position.y - s1.h / 2 <= s2.position.y - s2.h / 2) &&
              (s2.position.y - s2.h / 2 <= s1.position.y + s1.h / 2))))) {
        return 1;
    }
