        ...
        for (int i = 0; i < world->nb_bullets; i++) {
            if (distance(tmp_bullet->val, world->Rick) < 3 * SCREEN_HEIGHT) {
                ...
                if (tmp_bullet->val.dir == 3) {
                    ...
                    apply_sprite(renderer, textures->bullet_flipped, &tmp_bullet->val);
                }
            }
...
void clean(SDL_Window *window, SDL_Renderer *renderer, resources_t *textures, world_t *world, wav_device_t* wav_device_ptr) {
    clean_data(world);
    clean_textures(textures);
    clean_sdl(renderer, window);
    clean_audio(wav_device_ptr);
}
