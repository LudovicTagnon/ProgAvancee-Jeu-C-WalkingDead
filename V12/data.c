...
void play_audio(SDL_Event *event, SDL_Renderer *renderer, world_t *world, resources_t *textures, wav_device_t* wav_device_ptr){
    ...
    SDL_LoadWAV("wav/shotgun.wav", &wavSpec, &wav_device_ptr->wavBuffer, &wavLength);

    // open audio device

    wav_device_ptr->deviceId = SDL_OpenAudioDevice(NULL, 0, &wavSpec, NULL, 0);

    // play audio

    int success = SDL_QueueAudio(wav_device_ptr->deviceId, wav_device_ptr->wavBuffer, wavLength);
    SDL_PauseAudioDevice(wav_device_ptr->deviceId, 0);
...
void clean_audio(wav_device_t* wav_device_ptr){
    // clean up
    SDL_CloseAudioDevice(wav_device_ptr->deviceId);
    SDL_FreeWAV(wav_device_ptr->wavBuffer);
