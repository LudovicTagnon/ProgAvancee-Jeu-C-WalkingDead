...
void init_data(world_t *world) {
    ...
    //=======================================================================
    init_sprite_ptr(&world->Rick, SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, RICK_WIDTH/world->zoom, RICK_HEIGHT/world->zoom, 10,10,Rick);
    //=======================================================================

    for (int i = 0; i < NUMBER_ROADS; i++) {

        int x = 0;
        int y = (i * 1000)/world->zoom - (1000 * NUMBER_ROADS )/(2*world->zoom);

        sprite_t tmp_sprite;
        tmp_sprite = init_sprite(tmp_sprite, x, y, 1000/world->zoom, 1000/world->zoom, 1,1, Road);
    ...
    for (int i = 0; i < NUMBER_ZOMBIES; i++) {
        ...
        sprite_t tmp_sprite;
        tmp_sprite = init_sprite(tmp_sprite, x, y, 134/world->zoom, 180/world->zoom, 2,3, Zombie);
    ...
    for (int i = 0; i < NUMBER_LIFES; i++) {
        ...
        sprite_t tmp_vies;
        sprite_t tmp_faim;
        tmp_vies = init_sprite(tmp_vies, SCREEN_WIDTH - (i + 1) * 32, 16, 32, 32, 0,1, Barre);
        tmp_faim = init_sprite(tmp_faim, SCREEN_WIDTH - (i + 1) * 32, 56, 32, 32, 0,1, Barre);
    ...
    for (int i = 0; i < NUMBER_GUNS; i++) {
        ...
        sprite_t tmp_sprite;
        tmp_sprite = init_sprite(tmp_sprite, x, y, 90/world->zoom, 23/world->zoom, 1,1, Gun);
    ...
    for (int i = 0; i < NUMBER_TREES; i++) {
        ...
        tmp_sprite = init_sprite(tmp_sprite, x, y, 836/world->zoom, 1000/world->zoom, 1,1, Tree);
    ...
    for (int i = 0; i < NUMBER_CARS; i++) {
        ...
        if (d == 0 || d == 2) {
            tmp_sprite = init_sprite(tmp_sprite, x, y, 300/world->zoom, 540/world->zoom, 1,1, Car);
        } else if (d == 1 || d == 3) {
            tmp_sprite = init_sprite(tmp_sprite, x, y, 540/world->zoom, 300/world->zoom, 1,1, Car);
        }
    ...
    for (int i = 0; i < NUMBER_CHICKEN; i++) {
        ...
        sprite_t tmp_sprite;
        tmp_sprite = init_sprite(tmp_sprite, x, y, 140/world->zoom, 140/world->zoom, 3,1, Chicken);
...
void update_data(world_t *world, resources_t *textures, SDL_Renderer *renderer) {
    ...
    if(SDL_GetTicks() % 10000 > 9990 ){
        if(world->nb_faim_restant >= 1 && world->nb_vies_restantes >= 1){
    ...
    if(world->rick_dead==1 && (time(NULL) - world->time_mort > 5)){
        world->gameover=1;
    }
}
    ...
    while (tmp_zombie != NULL) {
        if (distance(tmp_zombie->val, world->Rick) < 3 * SCREEN_HEIGHT) {
            if (tmp_zombie->val.life > 0) {
                if (distance(tmp_zombie->val, world->Rick) < 600 && tmp_zombie->val.life != 0 && world->nb_vies_restantes>0) {
