...
void refresh_graphics(SDL_Renderer *renderer, world_t *world, resources_t *textures) {
    ...
    int tree_flag; // rick devant ou derriere l'arbre
    if(world->c_d==0 && (world->Rick.h + world->Rick.position.y  > 600 + world->current_tree->position.y)  ){
        tree_flag = 1; // rick devant l'arbre
    }
    else if(world->c_d==1 && (world->Rick.h + world->Rick.position.y  > 900 + world->current_tree->position.y)){
        tree_flag = 1; // rick devant l'arbre
    }
    else {
