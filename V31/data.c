...
void init_data(world_t *world) {
    ...
    for (int i = 0; i < NUMBER_LIFES; i++) {
        ...
        sprite_t tmp_vies;
        sprite_t tmp_faim;
        tmp_vies = init_sprite(tmp_vies, SCREEN_WIDTH - (i + 1) * 32, 16, 32, 32, 0);
        tmp_faim = init_sprite(tmp_faim, SCREEN_WIDTH - (i + 1) * 32, 56, 32, 32, 0);
    ...
    for (int i = 0; i < NUMBER_CHICKEN; i++) {
        ...
        x = -(SCREEN_WIDTH * NUMBER_CHICKEN / 50) + rand() % (SCREEN_WIDTH * NUMBER_CHICKEN / 25);
        y = -(SCREEN_WIDTH * NUMBER_CHICKEN / 50) + rand() % (SCREEN_HEIGHT * NUMBER_CHICKEN / 25);

        sprite_t tmp_sprite;
        tmp_sprite = init_sprite(tmp_sprite, x, y, 140, 140, 2);
