...
void init_data(world_t *world) {
    ...
    for (int i = 0; i < NUMBER_CARS; i++) {
        ...
        int x = - (400 + rand() % (4*SCREEN_WIDTH));
        int y = -2*SCREEN_HEIGHT + rand() % (4*SCREEN_HEIGHT);

        int d = rand() % 4;

        sprite_t tmp_sprite;
        if(d==0 || d==2){
            tmp_sprite = init_sprite(tmp_sprite, x, y, 400, 720, 1);
        }else if(d==1 || d==3){
            tmp_sprite = init_sprite(tmp_sprite, x, y, 720, 400, 1);
        }
